	...
	<div class="panel-body">
		...
		<div class="table-responsive">
			<table
				...
				<tbody>
					<c:forEach var="list" items="${bookrequest_list}">
						<tr>
							<td>${list.book_name}</td>
							<td>${list.book_type}</td>
							<td>${list.user.first_name} ${list.user.last_name}</td>								
							<td><a
								href="BookRequestController?command=messageUser&id=${list.id}">
									<button name="submit" class=" btn btn-lg btn-primary">Message</button>
							</a></td>
							<td><a
								href="BookRequestController?command=deleteBookRequest&id=${list.id }">
