	...
	<div class="panel-body">
		...
		<div class="table-responsive">
			<table
				...
				<tbody>
					<c:forEach var="list" items="${lending_request_list}">
						<tr>
							<td>${list.book.book_name}</td>
							
							<td>${list.user.first_name} ${list.user.last_name}</td>								
							<td><a
								href="LendingReqController?command=LendBook&id=${list.id}&bk_id=${list.book.id}&usr_id=${list.user.id}">
									<button name="lend" class=" btn btn-lg btn-danger">Lend</button>
							</a></td>
						</tr>
					</c:forEach>
